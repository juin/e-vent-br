...
class PersistenciaCertificado extends InstanciaUnica {
	...
	public function selecionarPorUsuarioEvento($cod_usuario, $cod_evento) {
		$sql = 'Select a.nome_certificado, b.nome as nome_atividade, b.carga_horaria, 
			  ...
			  d.cod_atividade_agenda=f.cod_atividade_agenda AND f.cod_atividade=
        		b.cod_atividade AND d.frequente="Presente" AND a.cod_usuario=' . $cod_usuario . ' AND c.cod_evento = ' . $cod_evento;
	...
	public function enviarCertificado($cod_usuario) {
		$sql = 'SELECT u.email, u.cod_usuario, u.nome_certificado, c.url_certificado
				from Usuario u, Certificado c
				WHERE u.cod_usuario=' . $cod_usuario . ' AND c.cod_inscricao = (Select cod_inscricao from Inscricao 
		...
		foreach ( $registros as $registro ) {
			...
			$usuario->setUrlCertificado ( $registro ['url_certificado'] );
			$usuarios [$i] = $usuario;
			$i ++;
		}
		return $usuarios;
	...
	public function salvarCertificado(Certificado $certificado) {
		...
		$sql = "INSERT INTO `certificado`(`cod_certificado`, `data_hora_emissao`, 
				...
				`cod_inscricao`) VALUES (" . $certificado->getCodCertificado () . "," . $certificado->getData_hora_emissao () . "," . date ( 'Y-m-d' ) . "," . $certificado->getCodValidacao () . "," . $certificado->getCodInscricao () . ")";
